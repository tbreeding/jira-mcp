...
export function updateWizardState(
	...
	log(`DEBUG: Partial state to apply: ${JSON.stringify(partialState)}`)
	...
	if (isDifferentState(currentState, partialState)) {
		...
		// Use the state machine to handle the transition
		const transitionResult = transitionState(currentState, partialState.currentStep as WizardStep)
		...
		const validationResult = validateStateTransition(currentState, partialState)
